	...
	d_new_entries(0)
{
...
#endif
	// Open the database used to control per-process cache accesses
	...
	// Open the file used to store the size of the cache.
	...
	// Write lock the 'size file' and set the size to 0 if the file has just
	// been created.
...
HTTPCacheTable::CacheEntry *
HTTPCacheTable::read_entry(const string &url)
{
	db_rewind(d_db);
...
/** Get a pointer to a CacheEntry from the cache table.
    ...
    @param url Look for this URL. */
HTTPCacheTable::CacheEntry *
HTTPCacheTable::read_lock_entry(const string &url)
{
	db_rewind(d_db);
	char *data = db_start_read(d_db, url.c_str());
	HTTPCacheTable::CacheEntry *e = 0;	
	if (data)
		e = cache_index_parse_line(data);
	
	e->db = db_copy_handle(d_db);
	return e;
...
/** Find the CacheEntry for the given url and remove both its information in
	...
	@todo Change the name of this - it really removes the entry from both 
	the disk cache and the database entry.
