...
void Implicit0Cells::print(size_t indent) {
  ...
  cout << "<CELLARRAY>: \n";
  for (i=0; i<indent; i++) cout << " ";
  cout << "size: " << ca->getsize() << "\n";
  cout << "nodecount: " << ca->getNodeCount() << "\n";
  cout << "start, stop, step: " << this->start << this->stop << this->step << endl;
  for (i=0; i<indent; i++) cout << " ";
  cout << "cells: \n";
    cout << "(implicit)" << endl;
}
...
void Implicit0Cells::toNodeSet(set<Node> &outset) {
  for (size_t i=this->start; i<this->stop; i+=this->step) {
    outset.insert(i);
  }
}

Cell Implicit0Cells::getCellCopy(idx i) {
  assert(this->contains((Node) i));
  Cell c(1);
  c.getnodes()[0] = i;
  return c;
}
...
// This is broken.  A NodeMap should be a function from CellArray to CellArray.
// NodeMaps should only operate at the grid level
// But one fix/hack would be to let this class materialize its nodes when necessary
