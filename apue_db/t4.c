...
#define key1 "betabetabetabetabetabeta"
	...
	int status = db_is_key_locked(db, key1);
	fprintf(stderr, "db record status (betabeta...) %d\n", status);
...
#define key2 "AlphaAlpha"
	...
	status = db_is_key_locked(db, key2);
	fprintf(stderr, "db record status (AlphaAlpha...) %d\n", status);
